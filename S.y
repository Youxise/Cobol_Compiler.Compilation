...
%token mc_ident mc_data mc_prog mc_work mc_proc mc_stopr mc_et mc_ou mc_non mc_sup mc_inf mc_supe
 mc_infe mc_equi mc_diff mc_const <str>mc_int <str>mc_float <str>mc_char <str>mc_str mc_line mc_size mc_acc mc_type
...
LIST_IDF : idff virg LIST_IDF { strcpy(vars[j],$1); //printf("vars[j] =%s\n",vars[j]);
                                if (doubleDeclaration($1)==0) printf ("<< Erreur semantique ( Idf non declaree ), ligne %d, colonne %d : %s >>\n",yylineo,col,$1);
                                                           
                              j++;}
         | idff {strcpy(vars[j],$1); //printf("vars[j] =%s\n",vars[j]);
                if (doubleDeclaration($1)==0) printf ("<< Erreur semantique ( Idf non declaree ), ligne %d, colonne %d : %s >>\n",yylineo,col,$1);
                j++;}
;
...
CSTE : csti { //printf("csti=%d\n",$1);
              if(($1==0) && (a==1)) {printf("<< Erreur semantique ( division par 0 ), ligne %d, colonne %d : %d >>\n",yylineo,col,$1); a=0;}
              if(a==1) a=0;
              //if((b==1) && strcmp((char*)TypeEntite($1),cstype)!=0)  {printf("Erreur semantique: incompatibilite de type, ligne %d, colonne %d \n",nb,col); b=0;strcpy(cstype,"");}
              //itoa($1, buf, 10); 
              sprintf(buf,"%d",$1); $$=buf;
     ...
     |  cstf { //printf("cstf=%f\n",$1);
              if(($1==0) && (a==1)) {printf("<< Erreur semantique ( division par 0 ), ligne %d, colonne %d : %f >>\n",yylineo,col,$1); a=0;}
              if(a==1) a=0;
              //if((b==1) && strcmp((char*)TypeEntite($1),cstype)!=0) {printf("Erreur semantique: incompatibilite de type, ligne %d, colonne %d \n",nb,col);b=0;strcpy(cstype,"");}
              //gcvt($1, 10, buf); 
              sprintf(buf,"%f",$1);  $$=buf; 
